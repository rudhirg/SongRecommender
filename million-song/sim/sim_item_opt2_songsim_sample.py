...
def GetItemBasedRecommendations(user, alpha, gamma, songSimCalculator):
    k = 70000;
    gItemSamples = random.sample(gTrainingItems, k);
	#find the similarity with all users as preprocessing step
	rankVect = {};
	for relSid in gItemSamples:
		Wuv = 0.0;
		...
		for i in user.m_features.iterkeys():
			...
			#Wuv += GetSongSimilarityWeight(item, relItem, alpha, gamma);
			Wuv += songSimCalculator.calculate_similarity(item, relItem);
		rankVect[relSid] = Wuv;
...
def CalculateMeanAveragePrecision(userList, startIndx, unitLen, alpha, gamma, recFileName, songSimCalculator):
	numCount = 0;
	totalPrec = 0.0;
	...
	for i in range(startIndx, startIndx+unitLen):
		...
		itemVecSorted = GetItemBasedRecommendations(user, alpha, gamma, songSimCalculator);
...
class SimThread(multiprocessing.Process):
	def __init__(self,  userList, startIndx, unitLen, alpha, recFileName, gamma, queue, songsimCalc):
	...
	def run(self):
		...
		self.precision = CalculateMeanAveragePrecision(self.userList, self.startIndx, self.unitLen, self.alpha, self.gamma, self.recFileName, self.simCalculator);
...
def RunThreads(alpha, datalen, headline, gamma, songsimSharedData):
	...
	while(True):
		...
		t = SimThread( userList, startIndx, unitLen, alpha, recFileName, gamma, resultQueue, songSimCalc);
		t.daemon = True;
		t.start();
		threadList.append(t);
