...
gTestingUsers = {};
gTrainingItems = {};
gItemSamples = {}; # sampling over the whole set
...
def GetItemBasedRecommendations(user, alpha, gamma):
...
def CalculateMeanAveragePrecision(userList, startIndx, unitLen, alpha, gamma, recFileName):
    ...
    for i in range(startIndx, startIndx+unitLen):
        ...
        itemVecSorted = GetItemBasedRecommendations(user, alpha, gamma);
...
def RunThreads(alpha, datalen, headline, gamma):
    ...
    gResultsFile.write("item based:: alpha: %f, gamma,: %d, precision: %f\n" % (alpha, gamma, totalPrec/Len));
    gResultsFile.flush();
